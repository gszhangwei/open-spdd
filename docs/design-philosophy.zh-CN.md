...
这两个维度有交叉但不完全重叠。能力进步确实可以缓解部分控制问题——更大的上下文窗口、持久记忆、更好的对话式澄清，都在改善 AI 对设计意图的理解。但有一类控制需求，仅靠能力提升无法消除：当存在多种"正确"方案时，选哪一种是人类的决策，不是 AI 的推断；而这个决策需要被记录、共享和持续维护，否则下一次对话、下一个团队成员、下一个 Agent 都需要重新做一遍。

一个资深架构师，能力毋庸置疑。为什么企业仍然要求他写设计文档？不是因为他能力不足，而是因为团队需要共享理解、决策需要可追溯、未来的维护者需要上下文。同理，飞机自动驾驶再智能，飞行计划也不会消失——不是因为自动驾驶不够好，而是因为空管、机组、调度需要一个共享的、可验证的协作基准。

控制问题并不是 AI 编码时代才出现的新命题。Fred Brooks 在 1986 年区分"本质复杂度"和"偶然复杂度"时，就指出软件最难的部分不是编码，而是规格与设计；ADR（Architecture Decision Record）、Design by Contract 等实践，也都在解决"如何外化和传达设计决策"。

但 AI 编码让这个老问题变得更紧迫——不是因为返工的绝对成本更高（AI 重新生成很快），而是因为偏差更隐蔽。AI 生成的代码通常质量不低、风格一致、能通过编译和基础测试，很容易通过粗略的 review。设计方向上的偏移——用了错误的架构模式、多做了不该做的事——往往要到后续迭代甚至上线后才被发现，那时修正成本才真正高昂。
...
OpenSPDD 提出的 REASONS Canvas 尝试做的，是把这些分散的关注点整合成一个可嵌入 AI 编码工具链的统一框架——一个 7 维度的 checklist：
...
还有一个必须正面回应的问题：**结构化提示词自身也会过时。** 前文用"v1.0 的 Plan 到 v1.1 已经过时"来论证设计意图容易丢失，但同样的质疑完全适用于 REASONS Canvas——如果代码改了而 Canvas 没跟着更新，它就从"设计指南"变成了"误导性的过时文档"，比没有文档更危险。

这不是一个可以轻描淡写的问题。坦率地说，设计规格与代码的双向同步是当前最大的工程难题之一，尚未被任何方案完全解决。我们目前的做法是：在每次功能迭代后，手动运行 `/spdd-sync` 让 AI 检测代码变更并更新对应的 Canvas。这还不是一个强制性的流程，更多是团队约定——"改完代码，顺手同步规格"。它不完美，但在实践中行之有效：关键决策的变更基本能被捕捉到，Canvas 不至于严重过时。未来的方向是把这一步自动化——比如每次代码更新后，自动触发同步检测，如果发现变更，则自动更新对应的 Canvas。
...
也许未来它不再叫 prompt。也许它会演变成 DSL、决策记录、可执行 spec、自动同步的契约文件。但它背后的需求不会消失。
...
如果你想尝试 REASONS Canvas 这种更结构化的方式：
